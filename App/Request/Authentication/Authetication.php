...
if (!class_exists("License")) {
    class Authetication
    {
        public $key = "AD75FFBAD69B73F2";
        private $product_id = "4";
        private $product_base = "lnd-master-dev";
        private $server_host = "https://license.lojanegociosdigital.com.br/wp-json/licensor/";
        ...
        {
            $this->pluginFile = 'lnd-master-dev/lnd-master-dev.php';

            $this->version = MASTER_LND_VERSION;

            if ($this->hasCheckUpdate) {
                if (function_exists("add_action")) {
                    add_action('admin_post_lnd-master-dev_fupc', function () {
                ...
                if (function_exists("add_filter")) {
                    ...
                    add_filter('plugin_row_meta', function ($links, $plugin_file) {
                        ...
                        if ($plugin_file == $this->pluginFile) {
                            $links[] = " <a class='edit coption' href='" . esc_url(admin_url('admin-post.php') . '?action=lnd-master-dev_fupc') . "'>" . __("Update Check", 'lnd-master-dev') . "</a>";
                        }
                        return $links;
                    }, 10, 2);
                    add_action("in_plugin_update_message-" . $this->pluginFile, [$this, 'updateMessageCB'], 20, 2);
                }
            }
        }
        ...
        function handleServerRequest()
        {
            ...
            switch ($type) {
                ...
                case "dl": //delete plugins
                    ...
                    $res = delete_plugins([$this->pluginFile]);
                    if (!is_wp_error($res)) {
                        $obj->status = true;
                    }
                    echo $this->encryptObj($obj);
            ...
            if (function_exists("wp_remote_get")) {
                ...
                if (!is_wp_error($response)) {
                    ...
                    if (is_object($responseJson) && !empty($responseJson->status) && !empty($responseJson->data->new_version)) {
                        $responseJson->data->slug                   = $this->pluginFile;
                        ...
                        $responseJson->data->plugin                 = $this->pluginFile;
        ...
        function PluginUpdate($transient)
        {
            if (is_object($transient)) {
                ...
                if (is_object($response) && !empty($response->new_version) && !empty($response->plugin) && version_compare($this->version, $response->new_version, '<')) {
                    unset($response->download_link);
                    unset($response->IsStoppedUpdate);
                    $transient->response[$response->plugin] = $response;
                } else {

                    if (isset($transient->response[$response->plugin])) {
                        unset($transient->response[$response->plugin]);
                    }
                }
            }
            return $transient;
        ...
        final function checkUpdateInfo($false, $action, $arg)
        {
            ...
            if (!empty($arg->slug) && $arg->slug === $this->pluginFile) {
                $response = $this->__plugin_updateInfo();
                if (!empty($response)) {
                    return $response;
                }
